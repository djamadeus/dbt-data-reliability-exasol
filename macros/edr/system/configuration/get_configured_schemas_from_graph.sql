{% macro get_configured_schemas_from_graph() %}
    {{ return(adapter.dispatch('get_configured_schemas_from_graph','elementary')()) }}
{% endmacro %}

{% macro default__get_configured_schemas_from_graph() %}
...
{% macro exasol__get_configured_schemas_from_graph() %}
    {% set configured_schemas = [] %}
    {% set existing_schemas = [] %}
    {% if execute %}
    {% set root_project = context["project_name"] %}
